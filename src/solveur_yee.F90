...
!Ex[0:mx,0:my+1,]*Ey[0:mx+1,0:my] -> Bz[0:mx,0:my]
...
do i = ix, jx-1
   tm%bz(i,jy) = tm%bz(i,iy)
end do
do j = iy, jy-1
   tm%bz(jx,j) = tm%bz(ix,j)
end do

tm%bz(jx,jy) = tm%bz(ix,iy)

do i = ix, jx
   dbz_dy = (tm%bz(i,iy)-tm%bz(i,jy-1)) / dy
   tm%ex(i,iy) = tm%ex(i,jy) + dt*csq*dbz_dy 
end do
     
do j = iy, jy
   dbz_dx = (tm%bz(ix,j)-tm%bz(jx-1,j)) / dx
   tm%ey(ix,j) = tm%ey(jx,j) - dt*csq*dbz_dx 
end do
